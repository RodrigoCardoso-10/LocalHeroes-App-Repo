...
// Define a basic interface for a Job (you should align this with your backend data)
interface Job {
  _id: string;
  title: string;
  description: string;
  ...
  createdAt: string;
  updatedAt: string;
  // Add other relevant job properties like applicant count, creation date, etc.
}
...
const MyPostedJobsScreen = () => {
  ...
  const fetchPostedJobs = React.useCallback(async () => {
    ...
    try {
      ...
      const response: Job[] = await authService.getMyPostedJobs(user.id); // Use user.id here
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.centerContainer}>
          ...
          <Text style={styles.emptyDescription}>
            You haven't posted any jobs yet. Start helping your community!
          </Text>
        </View>
      </SafeAreaView>
    );
