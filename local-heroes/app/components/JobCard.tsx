...
const JobCard: React.FC<JobCardProps> = ({ job, onPress }) => {
  ...
  const getLocationAddress = (location: JobCardProps['job']['location']): string => {
    if (!location) {
  ...
  const firstName = typeof poster.firstName === 'string' ? poster.firstName : 'Unknown';
    ...
    <View style={styles.card}>
      ...
      <View style={styles.footerRow}>
        ...
        <View style={[styles.statusBadge, getStatusBadgeStyle(job.status)]}>
          <Text style={[styles.statusText, getStatusTextStyle(job.status)]}>
            {typeof job.status === 'string' ? job.status.replace('_', ' ') : 'Unknown'}
          </Text>
        </View>
      </View>
      {job.tags && job.tags.length > 0 && (
        <View style={styles.tagsRow}>
          ...
          {job.tags.length > 3 && <Text style={styles.moreTagsText}>+{job.tags.length - 3}</Text>}
        </View>
      )}
